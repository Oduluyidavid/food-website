...
const Header = () => {
  ...
  return (
    <nav className="flex justify-between items-center gap-4 bg-slate-800 lg:px-10 px-4 py-6 sticky top-0 z-30">
      <div id="logo">
        <h1 className="lg:text-[35px] text-[28px] text-orange-500 font-bold">MY FOODIFY</h1>
      </div>
      ...
      <div className={`${isMenuOpen ? 'flex' : 'hidden'} w-full h-fit bg-slate-800 p-4 absolute top-[80px] left-0 `} onClick={closeMenu}>
        <ul className='flex flex-col justify-center items-center gap-2 w-full'>
          {navItems.map(({ link, path }) => (
            <Link key={path} to={path} spy={true} offset={-100} smooth={true} className='text-white uppercase font-semibold cursor-pointer p-3 rounded-lg
             hover:bg-orange-500 hover:text-black w-full text-center'>{link}</Link>
          ))}
        </ul>
      </div>
